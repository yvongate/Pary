...
def get_upcoming_matches_from_fixtures(time_window_minutes: int = 30, buffer_minutes: int = 5) -> List[Dict]:
    """
    Lit fixtures.csv et récupère les matchs qui commencent dans ~30 min

    Args:
        time_window_minutes: Temps avant le match (défaut: 30 min)
        buffer_minutes: Marge de tolérance (défaut: 5 min = fenêtre 25-35 min)
