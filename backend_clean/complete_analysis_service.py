...
class CompleteAnalysisService:
    ...
    def analyze_match(self, home_team: str, away_team: str, league_code: str,
        ...
        shots_interval = self._calculate_confidence_interval(
            prediction_result['predictions'].get('home_team_shots', 0) +
            prediction_result['predictions'].get('away_team_shots', 0),
            prediction_result.get('confidence', {}).get('shots_log_likelihood', 50) / 100
        )

        corners_interval = self._calculate_confidence_interval(
            prediction_result['predictions'].get('home_corners', 0) +
        ...
        result = {
            ...
            'predictions': {
                'shots': {
                    ...
                    'confidence': prediction_result.get('confidence', {}).get('shots_log_likelihood', 50) / 100,
                    'predicted_value': prediction_result['predictions'].get('total_shots', 0)
                },
                'corners': {
