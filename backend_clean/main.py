...
async def process_full_prediction(
    ...
    try:
        ...
        if has_full_ai_analysis:
            ...
            shots_range = {
                'min': prediction_result['predictions'].get('home_shots_range', {}).get('min', int(home_shots + away_shots - 5)),
                'max': prediction_result['predictions'].get('home_shots_range', {}).get('max', int(home_shots + away_shots + 5))
            }
            ...
            if bookmaker_props_text and bookmaker_props_text.strip():
                ...
                context = {
                    ...
                    'home_stats': {'avg_shots': home_shots, 'avg_shots_on_target': prediction_result.get('analysis', {}).get('home_team', {}).get('avg_shots_on_target', home_shots * 0.45)},
                    'away_stats': {'avg_shots': away_shots, 'avg_shots_on_target': prediction_result.get('analysis', {}).get('away_team', {}).get('avg_shots_on_target', away_shots * 0.45)},
            ...
            context = {
                ...
                'home_players': [p.strip() for p in home_players.split(',')] if home_players else [],
                'away_players': [p.strip() for p in away_players.split(',')] if away_players else [],
                'home_stats': {'avg_shots': home_shots, 'avg_shots_on_target': prediction_result.get('analysis', {}).get('home_team', {}).get('avg_shots_on_target', home_shots * 0.45) if prediction_result and 'error' not in prediction_result else home_shots * 0.45},
                'away_stats': {'avg_shots': away_shots, 'avg_shots_on_target': prediction_result.get('analysis', {}).get('away_team', {}).get('avg_shots_on_target', away_shots * 0.45) if prediction_result and 'error' not in prediction_result else away_shots * 0.45},
                'league': LEAGUES.get(league_code, {}).get('name', 'Unknown'),
                'match_date': datetime.now().isoformat(),
                'bookmaker_propositions': bookmaker_props_text,
                # Classements et stats détaillées
        ...
        print(f"[PREDICTION {prediction_id}] IA Deep: Total tirs {shots_range}")
